...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@300;400;500;600&family=Inter:wght@300;400;500;600;700&display=swap">
<style>
...
async function init() {
  ...
  // Time-limited: evolve as many steps as possible within budget per rule
  for (const r of RULES) {
    const d = DATA[r.id];
    if (!d || !d.states || !d.states[0]) continue;
    const parsed = parseNotation(r.notation);
    if (!parsed) continue;
    const totalSteps = d.states.length - 1;
    try {
      const result = HGEngine.evolve(d.states[0], parsed.lhs, parsed.rhs, totalSteps, 4000);
      // Replace ALL data with re-evolved result (may be fewer steps if timed out)
      d.states = result.states;
      d.events = result.events;
      d.causal_edges = result.causal_edges;
...
const HGEngine = (() => {
  ...
  function evolve(initHyp, lhs, rhs, steps, timeLimitMs) {
